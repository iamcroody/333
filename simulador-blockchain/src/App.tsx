                ...
                {isLogOpen ? (
                    ...
                    >
                        <button 
                            ...
                            className="absolute -right-3 top-10 w-6 h-6 bg-gold-600 rounded-full flex items-center justify-center text-black shadow-lg hover:bg-gold-400 transition-colors z-50"
                        >
                            <ChevronLeft size={14} />
                        </button>
                    ...
                    <motion.button 
                        ...
                        className="fixed left-6 top-32 w-12 h-12 bg-black border border-gold-500/30 rounded-full flex items-center justify-center text-gold-500 shadow-2xl z-50 hover:bg-gold-900/20 hover:border-gold-500 transition-all group"
                    >
                        <Menu size={20} className="group-hover:rotate-90 transition-transform duration-300" />
                        <div className="absolute -right-1 -top-1 w-3 h-3 bg-gold-500 rounded-full animate-pulse shadow-[0_0_10px_#d4a528]"></div>
                    </motion.button>
                )}
            </AnimatePresence>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 relative overflow-hidden">
                <AnimatePresence>
                    {hoveredNode && (
                        <motion.div 
                            ...
                            className="absolute top-6 right-6 w-64 bg-black/90 border border-gold-500/50 p-4 rounded shadow-2xl z-50 pointer-events-none"
                        >
                ...
                <div className="flex flex-col lg:flex-row items-center justify-center gap-12 w-full max-w-6xl">
                    <motion.div 
                        ...
                        className="relative w-[300px] h-[300px] sm:w-[450px] sm:h-[450px] flex items-center justify-center"
                    >
                        <motion.div 
                            ...
                            className={clsx(
                                "absolute w-40 h-40 sm:w-56 sm:h-56 rounded-full border backdrop-blur-3xl flex flex-col items-center justify-center z-10 transition-colors duration-500",
                        ...
                        {transactions.map((tx, i) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={clsx(
                                        "absolute bg-black/90 border p-2 sm:p-3 rounded shadow-2xl z-20 transition-all duration-500 group cursor-help",
                                        isInfected && i === 0 ? "border-red-500 shadow-[0_0_20px_rgba(220,38,38,0.5)] bg-red-900/20" : "border-gold-500/20"
                                    )}
                                >
                                    <div className={clsx("text-[6px] sm:text-[7px] mb-1 font-tech uppercase", isInfected && i === 0 ? "text-red-400" : "text-gold-700")}>
                                        {isInfected && i === 0 ? "MALICIOUS TX" : `TX #${i}`}
                                    </div>
                                    <div className={clsx("text-[8px] sm:text-[10px] font-bold", isInfected && i === 0 ? "text-red-500" : "text-gold-100")}>
                        ...
                        <svg className="absolute inset-0 w-full h-full pointer-events-none overflow-visible">
                            <motion.circle 
                                cx="50%" cy="50%" r={typeof window !== 'undefined' && window.innerWidth < 640 ? "120" : "180"} 
                    ...
                    {/* INTERACTIVE NODES - VERTICAL ON THE RIGHT */}
                    <div className="flex lg:flex-col gap-6 sm:gap-10 z-30">
                        {nodes.map((node) => (
                            <div 
                                key={node.id} 
                            ...
                            >
                                <div className={clsx(
                                    "w-12 h-12 sm:w-16 sm:h-16 rounded-2xl border flex flex-col items-center justify-center transition-all duration-700 relative z-10",
                                    node.status === 'idle' ? "bg-black/80 border-gold-900/30 text-gold-900 group-hover:border-gold-500/50" :
                                    node.status === 'verifying' ? "bg-gold-900/20 border-gold-400 text-gold-400 scale-110" :
                                    node.status === 'resonating' ? "bg-gold-500 border-black text-black scale-105 shadow-[0_0_20px_#d4a528]" :
                                    node.status === 'breach' ? "bg-red-950/40 border-red-500 text-red-500" :
                                    "bg-emerald-500 border-black text-black shadow-[0_0_20px_#10b981]"
                                )}>
                                ...
                                <div className="text-center hidden sm:block">
                                    ...
                                    <div className="text-[7px] text-neutral-600 uppercase mt-1 font-tech font-bold tracking-widest leading-none">{node.role}</div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                {/* CONTROLS */}
                <div className="mt-8 sm:mt-12 flex gap-4 w-full max-w-md z-30">
                    ...
                    <div className="w-[800px] bg-black/80 backdrop-blur border border-gold-900/30 p-8 rounded-lg shadow-2xl relative overflow-hidden">
                        ...
                        <div className="grid grid-cols-2 gap-8 relative z-10">
                             <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <select 
                                        className="w-full h-[52px] bg-black/60 border border-gold-900/30 px-4 text-xs font-tech text-gold-100 focus:outline-none focus:border-gold-500 transition-colors rounded appearance-none cursor-pointer hover:border-gold-500/50"
                             ...
                             <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" className="w-full h-[52px] bg-black/60 border border-gold-900/30 px-4 text-xs font-tech text-gold-100 focus:outline-none focus:border-gold-500 transition-colors rounded placeholder:text-neutral-800" />
                                </div>
                             </div>
                        </div>
                ...
                <>
                    <AnimatePresence initial={false}>
                        {isSidebarOpen ? (
                            ...
                            >
                                <Menu size={20} className="group-hover:rotate-90 transition-transform duration-300" />
                                <div className="absolute -right-2 -top-2 w-4 h-4 bg-red-500 rounded-full text-[8px] flex items-center justify-center text-white font-bold animate-bounce">!</div>
                            </motion.button>
                        )}
                    </AnimatePresence>
